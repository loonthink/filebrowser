<template>
  <div>
    <header-bar>
      ...
      <template #actions>
        ...
        <action
          ...
          @action="uploadFunc"
        />
        <action icon-name="info" :label="t('buttons.info')" show="info" />
        <!-- <action
          icon="check_circle"
    ...
    <div
      v-if="isMobile && !isInlineSearchActive"
      id="file-selection"
      :class="{
...
import Action from "@/components/header/NewAction.vue";
import Item from "@/components/files/NewListingItem.vue";
import ContextMenu from "@/components/ContextMenu.vue";
import InlineSearchInput from "@/components/search/InlineSearchInput.vue";
import InlineSearchResults from "@/components/search/InlineSearchResults.vue";
...
const inlineSearchInput = ref<{ focus: () => void } | null>(null);

const inlineSearchBasePath = computed(() => route.path);
const {
  clear: clearInlineSearch,
  isActive: isInlineSearchActive,
  ongoing: searchOngoing,
  query: searchQuery,
  results: searchResults,
} = useInlineSearch(inlineSearchBasePath, $showError);
...
const getSortIcon = (column: string) => {
  ...
  return fileStore.req.sorting.asc ? ascIcon : descIcon;
};

const nameIcon = computed(() => getSortIcon("name"));

const sizeIcon = computed(() => getSortIcon("size"));

const modifiedIcon = computed(() => getSortIcon("modified"));
...
watch(isInlineSearchActive, (active) => {
  fileStore.selected = [];
...
const sort = async (by: string) => {
  const isCurrentColumn = fileStore.req?.sorting.by === by;
  const asc = isCurrentColumn ? !Boolean(fileStore.req?.sorting.asc) : true;
